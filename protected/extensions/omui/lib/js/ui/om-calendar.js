...
;(function($) {
        ...
        _create : function() {
            ...
            if (opts.popup) {
                $self.wrap('<span></span>').after('<span class="om-calendar-trigger"></span>')
                    .parent().addClass("om-calendar om-widget om-state-default").css({position:'relative'});
                
                this.con = $('<div></div>').appendTo($self.parent()).css({
                        'top':'0px',
                        'position':'absolute',
                        'background':'white',
                        'visibility':'hidden',
	...
	$.extend($.om.omCalendar, {
		Page: function(config, father) {
            ...
            this.nav_html = [
                ...
                i18n.ok,
                '</button><button class="cancel" type="button">',
                i18n.cancel,
                '</button>',
                '</p>'
            ].join("");
	...
	$.extend($.om.omCalendar, {
        ...
        TimeSelector:function(ft, father) {
            ...
            this.ctime = $('<div class="om-cal-time om-state-default">' + i18n.time + ':<span class="h">h</span>:<span class="m">m</span>:<span class="s">s</span><!--{{arrow--><div class="cta"><button class="u om-icon om-icon-triangle-1-n" type="button"></button><button class="d om-icon om-icon-triangle-1-s" type="button"></button></div><!--arrow}}--></div>');
            this.button = $('<button class="ct-ok om-state-default" type="button">' + i18n.ok +'</button>');
            //小时
