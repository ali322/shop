<h2 class='category_bar_title category_bar_title_holder'><a href="<?php echo Yii::app()->createUrl('main/allCategory');?>">所有商品分类</a></h2>
<div class="category_droplist" style='display:none;'>
...
Yii::app()->clientScript->registerScript("categorynav","
    $('.category_bar_title_holder').parent().mouseenter(function(){
        $(this).find('.category_bar_title_holder').children().addClass('collapse');
        $(this).find('.category_droplist').show();
    }).mouseleave(function(){
        $(this).find('.category_bar_title_holder').children().removeClass('collapse');
        $(this).find('.category_droplist').hide();
    });
